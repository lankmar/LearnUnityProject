...
namespace BerserkAdventure
{
    public class Main : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // gameState = GameStage.MainMenu; //по идеи после этого должно запускатьс€ главное меню,  но пока сразу в игру
        ...
        private void GameQuest()
        {
            ...
            {
                if (item.GetComponent<QuestTextSettings>() != null) item.GetComponentInChildren<Text>().text = string.Format("¬ыберететесь из ловушки за {0}:{1}", ts.Minutes, ts.Seconds);
            }
        }
